...
# Set these in Render's dashboard under Environment, not here.
# ============================================================
...
_missing = [name for name, val in (("BOT_TOKEN", BOT_TOKEN), ("MONGO_URI", MONGO_URI), ("DB_NAME", DB_NAME)) if not val]
if _missing:
    raise RuntimeError(
        "Missing required environment variable(s): " + ", ".join(_missing) +
        ". Set them in Render's dashboard (Environment tab) before starting the service."
    )
...
# notifications get posted here. Leave unset/0 to disable logging entirely.
LOG_CHANNEL_ID = int(os.environ.get("LOG_CHANNEL_ID") or 0)
...
BOT_NAME = os.environ.get("BOT_NAME", "Telegram Assist Bot")
OWNER_URL = os.environ.get("OWNER_URL", "https://t.me/")
...
# Just the bot's @username, WITHOUT the leading "@" (e.g. "Filter_Assistbot").
BOT_USERNAME_FOR_ADD = os.environ.get("BOT_USERNAME_FOR_ADD", "").lstrip("@")
